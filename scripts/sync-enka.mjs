...
//
// Sources, from Enka's published store on the default branch:
// - store/characters.json: avatarId -> element, weapon, rarity, name hash, skill order
// - store/loc.json: text map hash -> localized string, 17 languages
//
...
// Both ids are the Traveler. The store splits them by starting element.
...
function buildChars(store, loc, byName, bySlug) {
  ...
  for (const [avatarId, meta] of Object.entries(store)) {
    // Hyphenated ids are Traveler element depots and never appear as an avatarId.
  ...
  return { chars, order, unmatched };
}

function buildSets(loc) {
  const want = new Map(ARTIFACT_SETS.map((s) => [s.toLowerCase(), s]));
  const sets = {};
  const covered = new Set();
  for (const [hash, value] of Object.entries(loc)) {
    if (typeof value !== "string") continue;
    const canonical = want.get(value.toLowerCase());
    if (!canonical) continue;
    covered.add(canonical);
    // A payload's flat.setNameTextMapHash is either this key or exactly 512 below it,
    // depending on the set. Emit both so the runtime lookup needs no fallback branch.
    sets[hash] = canonical;
    sets[String(Number(hash) - 512)] = canonical;
  }
...
async function main() {
  const [store, locAll] = await Promise.all([getJSON("characters.json"), getJSON("loc.json")]);
  const loc = locAll.en;
  if (!loc) throw new Error("loc.json has no English map");

  const byName = new Map(ROSTER.map((c) => [c.name.toLowerCase(), c]));
  const bySlug = new Map(ROSTER.map((c) => [c.slug, c]));
  const { chars, order, unmatched } = buildChars(store, loc, byName, bySlug);
  const { sets, missing } = buildSets(loc);
...
`;
  ...
  if (missing.length) console.log(`  no hash yet: ${missing.join(", ")}`);
